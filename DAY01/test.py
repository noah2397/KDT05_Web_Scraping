...
def scraping_use_find(html):
    ...
    print('[find 함수 사용]')
    print('총 tomestone-container 검색 개수:', len(bs.find_all('div', {'class': 'tombstone-container'})))
    print('-'*80)
...
def scraping_use_select(html):
    ...
    print('[select 함수 사용]')
    print('총 tomestone-container 검색 개수:', len(bs.select('div', {'class': 'tombstone-container'})))
    print('-' * 80)
